...
def test_hci_advertising_report_event_ad_data():
    ...
    assert(packet[EIR_CompleteLocalName].local_name == b'Pebble Time LE 71E6')
...
def test_hci_advertising_report_event_scan_resp():
    ...
    raw_mfg_data = b'\x00_B31147D2461\xfc\x00\x03\x0c\x00\x00'
    k = packet.getlayer(EIR_Manufacturer_Specific_Data)
    #print(dir(k) )
    assert(packet[EIR_Manufacturer_Specific_Data].load == raw_mfg_data) # .data has been .load
